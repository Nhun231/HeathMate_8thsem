...
import { getLatestNutrition, updateNutrition } from "../../services/UpdateNutrientService";
...
const NutritionDashboard = () => {
    ...
    const fetchData = async () => {
        try {
            const res = await getLatestNutrition();
            const data = res.data;
            setNutritionData(data);

            if (data?.tdee) {
                const carbPercent = ((data?.carbs ?? 0) * 4 / data.tdee) * 100;
                const proteinPercent = ((data?.protein ?? 0) * 4 / data.tdee) * 100;
                const fatPercent = ((data?.fat ?? 0) * 9 / data.tdee) * 100;
                const fiberPercent = ((data?.fiber ?? 0) * 2 / data.tdee) * 100;

                setCarbPercent(Number(carbPercent.toFixed(1)));
                setProteinPercent(Number(proteinPercent.toFixed(1)));
                setFatPercent(Number(fatPercent.toFixed(1)));
                setFiberPercent(Number(fiberPercent.toFixed(1)));
            }
        } catch (err) {
    ...
    const handleSave = async () => {
        ...
        if (totalPercent > 100) {
            setError("❌ Tổng % vượt quá 100%! Vui lòng giảm lại.");
            return;
        } else if (totalPercent < 100) {
            setError("⚠️ Tổng % thấp hơn 100%! Có thể bạn đang ăn thiếu năng lượng.");
        } else {
            setError("");
        }

        try {
            const { tdee } = nutritionData;

            const carbs = ((tdee * carbPercent) / 100 / 4).toFixed(1);
            const protein = ((tdee * proteinPercent) / 100 / 4).toFixed(1);
            const fat = ((tdee * fatPercent) / 100 / 9).toFixed(1);
            const fiber = ((tdee * fiberPercent) / 100 / 2).toFixed(1);
            ...
            console.error("Lỗi cập nhật nutrient:", err);
            setError(err.response?.data?.message || "Lỗi khi cập nhật dinh dưỡng!");
        }
    };
    ...
    return (
        <>
            ...
            <Typography variant="h5" align="center" color="text.secondary" mb={5}>
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    {/* Warning when over 100% and under 100% */}
                    {totalPercent > 100 && (
                    ...
                    {totalPercent < 100 && (
                        <Typography align="center" color="error" mt={2}>
                            ⚠️ Tổng % thấp hơn 100%! Có thể cơ thể bạn đang thiếu năng lượng.
                        </Typography>
                    )}
                </div>
