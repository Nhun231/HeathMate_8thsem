...
// Giả lập calculateAge và validateProfile (như trước)
...
const validateProfile = ({ name, gender, dob }: any) => {
  const errors: any = {};
  if (!name || typeof name !== 'string' || name.trim().length < 2 || name.trim().length > 64 || /[^a-zA-ZÀ-ỹ\s]/.test(name)) {
...
describe('UserService', () => {
  ...
  const testCases = [
    ...
    { name: 'A'.repeat(65), gender: 'Nữ', dob: '2000-01-01', shouldPass: false },
    ...
    { name: 'Nguyễn*Anh Tú', gender: 'Nam', dob: '2010-01-01', shouldPass: false },
  ];
  ...
  testCases.forEach((tc, idx) => {
    it(`UTCID0${idx + 1} should ${tc.shouldPass ? 'pass' : 'fail'}`, () => {
      ...
      if (tc.shouldPass) {
        expect(result).toEqual({ name: tc.name, gender: tc.gender, dob: tc.dob });
      } else {
        expect(['name', 'gender', 'dob'].some(prop => prop in result)).toBe(true);
      }
    });
  });
});
